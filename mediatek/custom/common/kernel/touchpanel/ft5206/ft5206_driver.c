				...
				u8 saddr, u8 *buf, int len)
{
	...
	if(client == NULL) {
		TPD_DMESG("%s:i2c_client is NULL\n", __func__);
		return -EINVAL;
	}
	if(buf == NULL) {
		TPD_DMESG("%s:buf is NULL\n", __func__);
		return -EINVAL;
	}
	...
	if (i2c_transfer(client->adapter, msgs, 2) < 0) {
		TPD_DMESG("%s:i2c_transfer failed!\n", __func__);
		return -EIO;
	}
...
static int ft5x0x_i2c_write_value(struct i2c_client *client,
 			    u8 saddr, u8 *writebuf, int writelen)
{
	int ret;
	u8 buffer[2];
...
static void tp_debug(int tp_debug_value);
			  ...
			  char *buf)
{
	...
	s += sprintf(s,"read tpswitch.\n");
	//s += sprintf(s, "%d",result);
...
{
#include "Ft5x16_Lib_Diablo_Ver0x12_ID0x5a_app.h"
};
...
static void tp_debug(int tp_debug_value)
{
	...
	int retry = 0;
   	...
   	if (tp_debug_value == 2) {
		...
		printk("touchpanel chip version is 0x%2xxxxxxxxxxxxx\n", chip_fw_ver); 
   		read_reg(0xa8, &factoryid);
		printk("touchpanel factory id is 0x%2xxxxxxxxxxxxx\n", factoryid);
		firmware_ver = fts_ctpm_get_upg_ver(); 
		printk("current firmware version is 0x%2xxxxxxxxxxxxx\n", firmware_ver);			
   	}
   	if (tp_debug_value == 4) {
   		ft5x0x_read_reg(i2c_client, 0xa6, &chip_fw_ver);		//firmware ID(0x??)   		
		printk("method 3 touchpanel chip version is 0x%2x xxxxxxxxxxxx\n", chip_fw_ver);   	
   		ft5x0x_read_reg(i2c_client, 0xa8, &factoryid);
		printk("method 3 touchpanel factory id is 0x%2x xxxxxxxxxxxx\n", factoryid);
   	}   
   	if (tp_debug_value == 6) {
   		ft5306_i2c_read_value(i2c_client, 0xa6, &chip_fw_ver, 1);		//firmware ID(0x??)
		printk("method 4 touchpanel chip version is 0x%2x  xxxxxxxxxxxx\n", chip_fw_ver);   		
   		ft5306_i2c_read_value(i2c_client, 0xa8, &factoryid, 1);		//firmware ID(0x??)
		printk("method 4 touchpanel factory id is 0x%2x  xxxxxxxxxxxx\n",factoryid);
   	} 
   	if (tp_debug_value == 8) {
   		i2c_smbus_read_i2c_block_data(i2c_client, 0xa6, 1, &chip_fw_ver); 
		printk("method smbus touchpanel chip version is 0x%2xxxxxxxxxxxxx\n", chip_fw_ver); 
   		i2c_smbus_read_i2c_block_data(i2c_client, 0xa8, 1, &factoryid); 
		printk("method smbus touchpanel factory id is 0x%2x xxxxxxxxxxxx\n", factoryid);
   	} 
   	if (tp_debug_value == 10) {   		
   		i2c_smbus_read_i2c_block_data(i2c_client, 0x00, 1, &workmode);
		printk("touchpanel workmode is 0x%2x xxxxxxxxxxxx\n", workmode);
   	}
}
