...
public:
    uint32_t getCharFontID(SkUnichar uniChar, uint32_t* glyphBase);
    uint16_t getCharGlyphIndex(uint16_t uniChar);
    unsigned getBaseGlyphCount() { return fBaseGlyphCount;};
    uint16_t getGlyphIDbyIndexAndFont(uint16_t glyphIndex, uint32_t fontID);
    void getFontMetricsByFontID(SkPaint::FontMetrics* mX, SkPaint::FontMetrics* mY, uint32_t fontID);
    void getFontMetricsByFontID(uint16_t* x_ppem, uint16_t* y_ppem, SkFixed* x_scale, SkFixed* y_scale, uint32_t fontID);
