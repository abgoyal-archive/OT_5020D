...
/** M: add font ID @{ */
typedef const SkGlyph& (*SkMeasureCacheByFontProc)(SkGlyphCache*, const char**, uint32_t fontID);
/** @} */
...
public:
    uint32_t getFontID(SkUnichar uniChar, uint32_t* glyphBase);
    /*
         * Convert text into glyph id.
         * Note:
         *   The text should be in the same font and two byte for one character (UCS2).
         *   The glyphs is based on the glyph index on the font.
         *   The length is UCS2 character count.
         *
      */
    int textToGlyphIndex(const void* text, size_t length, uint16_t glyphs[]) const;
    SkMeasureCacheByFontProc getMeasureCacheProcByFontID(TextBufferDirection tbd, bool needFullMetrics) const;
    SkScalar getFontMetricsByFontID(FontMetrics* metrics, uint32_t fontID, SkScalar scale = 0) const;
    void getFontMetricsByFontID(uint16_t* x_ppem, uint16_t* y_ppem, SkFixed* x_scale, SkFixed* y_scale, uint32_t fontID) const;
    /** @} */
};
