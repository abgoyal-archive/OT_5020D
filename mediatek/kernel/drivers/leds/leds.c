	...
	switch (cust->mode) {
		case MT65XX_LED_MODE_PWM:
			...
			}else
			{
				...
				{
#if defined(JRD_CAMRY_BORAD)
					if(!strcmp(cust->name,"button-backlight") ) //mingquan.lai modify for Camry
#else
...
//2013-01-09 huanhuan.guo "Enable led flicker when battery is low" begin
unsigned int flicker=0;//0 for flicker disabled, 1 for flicker enabled

int flicker_count=0;//could modify this variable to implement timeout

struct timer_list flicker_timer;

static void flicker_func()
{
	flicker_count++;
	
	if(flicker_count%2){//turn on led
		mt_set_gpio_mode(GPIO194, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO194, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO194, GPIO_OUT_ONE);
		
		flicker_timer.expires=jiffies+HZ/2;
	}else{//turn off led
		mt_set_gpio_mode(GPIO194, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO194, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO194, GPIO_OUT_ZERO);
		
		flicker_timer.expires=jiffies+HZ*3/2;
	}
	
	add_timer(&flicker_timer);
}

static ssize_t show_flicker(struct device *dev,struct device_attribute *attr,char *buf)
{
	sprintf(buf,"%d\n" ,flicker);
...
static int flicker_wake_lock = 0;

static ssize_t store_flicker(struct device *dev,struct device_attribute *attr,const char *buf,size_t size)
{
	char *pvalue = NULL;
	unsigned int value = 0;

	if(buf != NULL && size != 0){
		value = simple_strtoul(buf,&pvalue,0);
		
		if(value==0){//write int 0 disable flicker
			if(flicker==0)
				return size;
			
			del_timer(&flicker_timer);//remove timer
			...
			//release wake_lock
			if(flicker_wake_lock){
				wake_unlock(&leds_suspend_lock);
				flicker_wake_lock=0;
			}
		}else{//write int 1 enable flicker
			...
			add_timer(&flicker_timer);//start timer
...
static DEVICE_ATTR(flicker, 0664, show_flicker,store_flicker);
//2013-01-09 huanhuan.guo "Enable led flicker when battery is low" end
...
//mingquan.lai modify for Camry
//2013-01-11 huanhuan.guo "Change macro for flicker" begin
#if defined(JRD_CAMRY_BORAD)
           if(strcmp(g_leds_data[i]->cdev.name,"button-backlight") == 0){
#else
		if(strcmp(g_leds_data[i]->cdev.name,"red")==0){
			init_timer(&flicker_timer);
			flicker_timer.function=&flicker_func;
			flicker_timer.expires=jiffies+HZ/2;
		
			rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_flicker);
		    if(rc)
	            {
	                LEDS_DEBUG("[LED]device_create_file flicker fail!\n");
	            }
#endif	
//2013-01-11 huanhuan.guo "Change macro for flicker" end
