...
/** M: add xoffset and yoffset for GPOS  @{ */
void SkDraw::drawText(const char text[], size_t byteLength, const SkScalar* xOffset, const SkScalar* yOffset,
                      SkScalar x, SkScalar y, const SkPaint& paint) const {
    SkASSERT(byteLength == 0 || text != NULL);
    ...
    while (text < stop) {
        ...
        if (xOffset != NULL) {
            posX += SkFloatToFixed(xOffset[index]);
        }


        if (yOffset != NULL) {
            posY += SkFloatToFixed(yOffset[index]);
        }

        if (glyph.fWidth) {
            proc(d1g, posX, posY, glyph);
        }
        fx += glyph.fAdvanceX;
        fy += glyph.fAdvanceY;
        index ++;
