...
bool SkGIFImageDecoder::onDecode(SkStream* sk_stream, SkBitmap* bm, Mode mode) {
    ...
    if (length > MTK_MAX_SRC_GIF_FILE_SIZE) {
        SkDebugf("GIF: Unsupported Too large Gif source %d (> %d bytes)\n", length, MTK_MAX_SRC_GIF_FILE_SIZE);
        return false;
    }
    ...
    if(gif->SWidth*gif->SHeight > MTK_MAX_SRC_GIF_PIXELS) {
        SkDebugf("GIF : Unsupported Too Large dimension (%d %d) > %d!!", gif->SWidth, gif->SHeight, MTK_MAX_SRC_GIF_PIXELS);
        return error_return(gif, *bm, "Pixels > 1.5M");
    }
