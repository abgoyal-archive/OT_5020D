...
//mingquan.lai modify 20130125 for Camry
///////////////////////////////////////////////////////////////////////////////////////////
//// Extern Functions
///////////////////////////////////////////////////////////////////////////////////////////
...
#endif
//hongmei temp change for camry&megan,megan not use5405
...
#ifdef FEATURE_RECOVERY_FROM_HIGH_TEMPERATURE
       if( BATTERY_LOW_RECOVERY_CHARGING_TEMP <= BMT_status.temperature <= BATTERY_OVER_RECOVERY_CHARGING_TEMP) /*3<temp<43 charging recovery*/ 
               BMT_status.charger_protect_status = 0;	/* &= 0x001*/
#endif
...
//mingquan.lai modify 20130125 for Camry
    ...
    if (upmu_is_chr_det() == KAL_TRUE)
    {
        if ((BMT_status.bat_vol < RECHARGING_VOLTAGE) && (BMT_status.bat_full) && (g_HW_Charging_Done == 1))	
        {
            ...
            //}
            //BMT_status.bat_full = KAL_FALSE;    //mingquan.lai modify 20130125 for Camry
...
void BAT_thread_fan5405(void)
{
    ...
    fan5405_config_interface_liao(0x00,0xC0);	/*reset t32 and enable STAT pin low when charging*/		//mingquan.lai modify 20130125 for Camry
...
{
    #define BATTERY_BAR 10	/*jrd change from 25 --> 10*/	//mingquan.lai modify 20130125 for Camry
...
//Read HW Raw Data
    ...
    m=0;
    temp_val = pmic_bank1_read_interface(0x6A, &temp_val, 0xFF, 0x0);
    ...
    {        
        ...
        if(m>1000)
        {
                        printf("Power/Battery", "[fgauge_read_current] fg_get_data_ready_status timeout 1 !\r\n");
            break;
        }
	 temp_val = pmic_bank1_read_interface(0x6A, &temp_val, 0xFF, 0x0);
        temp_val = (temp_val & 0x04) >> 2;
    ...
    {        
        ...
        if(m>1000)
        {
            printf("Power/Battery", "[fgauge_read_current] fg_get_data_ready_status timeout 2 !\r\n");
            break;
        }
	 temp_val = pmic_bank1_read_interface(0x6A, &temp_val, 0xFF, 0x0);
        temp_val = (temp_val & 0x04) >> 2;
    ...
    dvalue = (kal_uint32) ((Temp_Value * 158122) / 100000);       
    return dvalue;
}
...
{    
    ...
    kal_uint32 fg_current=0;	//mingquan.lai modify 20130125 for Camry
...
//mingquan.lai modify 20130125 for Camry
		        ...
		        {
		     mt65xx_backlight_off();	//mingquan.lai modify 20130125 for Camry
			 ...
			 //mingquan.lai modify 20130125 for Camry
		      pchr_turn_off_charging_fan5405();
		      mt6577_sleep(600, KAL_FALSE);
		      BMT_status.bat_vol = PMIC_IMM_GetOneChannelValue(AUXADC_BATTERY_VOLTAGE_CHANNEL,10);
		      fg_current = fgauge_read_current();
 	             i = 0;
                     while( fg_current == 0 )
                       {
                     if(i > 1)
...
#if defined(TELSA_BAT_OCV) 
                    UbVbat = BMT_status.bat_vol+50;
#elif defined(DIABLOHD_BAT_OCV)
                    UbVbat = BMT_status.bat_vol;
#else
		      UbVbat = BMT_status.bat_vol+25;
#endif 
		      UbVbat |= fg_current <<15;
		      UbVbat = UbVbat |(1<<31);
		      pchr_turn_on_charging_fan5405();
                    printf("Restore brightness:%d,%d\r\n",BMT_status.bat_vol,fg_current);
                    mt65xx_leds_brightness_set(6, 255);
                    mt65xx_backlight_on();
#endif				
                
                    pchr_turn_off_charging_fan5405();	//jrd disable charging when boot kernel for fuel gauge

                    sc_force_ap_control(false);		/*diablo new version add these sentences FIXME*/
                    reset_md();
    ...
    else	/*boot when no charger on*/
    {
        ...
        {
			//mingquan.lai modify 20130125 for Camry
            BMT_status.bat_vol = PMIC_IMM_GetOneChannelValue(AUXADC_BATTERY_VOLTAGE_CHANNEL,15);
            fg_current = fgauge_read_current();
            i = 0;
            while( fg_current == 0 )
                {
...
#if defined(TELSA_BAT_OCV) 
            UbVbat = BMT_status.bat_vol+65;
#elif defined(DIABLOHD_BAT_OCV)
            UbVbat = BMT_status.bat_vol;
#else
	     UbVbat = BMT_status.bat_vol+30;            		       
#endif  
            UbVbat |= fg_current <<15;
            printf("[BATTERY] battery voltage(%dmV),UbVbat%d >= CLV I=%d! Boot Linux Kernel !! \n\r",BMT_status.bat_vol,UbVbat,fg_current);
            sc_mod_exit();
            return;
        }
        else
