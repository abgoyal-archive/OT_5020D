...
const SkGlyph& SkGlyphCache::getGlyphIDMetricsByFontID(uint16_t glyphIndex, uint32_t fontID) {
    SkScalerContext* ctx = fScalerContext;
    uint16_t glyphID = (uint16_t)fScalerContext->getGlyphIDbyIndexAndFont(glyphIndex, fontID);
    return getGlyphIDMetrics(glyphID);

}


const SkGlyph& SkGlyphCache::getGlyphIDAdvanceByFontID(uint16_t glyphIndex, uint32_t fontID) {
    SkScalerContext* ctx = fScalerContext;
    uint16_t glyphID = (uint16_t)fScalerContext->getGlyphIDbyIndexAndFont(glyphIndex, fontID);
    return getGlyphIDAdvance(glyphID);
}


size_t SkGlyphCache::getFontID(SkUnichar uniChar, size_t* glyphBase) {
    SkScalerContext* ctx = fScalerContext;

    size_t id = (size_t)fScalerContext->getCharFontID(uniChar, glyphBase);
    return id;
}

uint16_t SkGlyphCache::unicharToGlyphIndex(uint16_t uniChar) {
    SkScalerContext* ctx = fScalerContext;

    uint16_t id = (uint16_t)fScalerContext->getCharGlyphIndex(uniChar);
    return id;
}

void SkGlyphCache::getFontMetricsByFontID(uint16_t* x_ppem, uint16_t* y_ppem, SkFixed* x_scale, SkFixed* y_scale, uint32_t fontID) const {
    SkScalerContext* ctx = fScalerContext;

    fScalerContext->getFontMetricsByFontID(x_ppem, y_ppem, x_scale, y_scale, fontID);
}

const SkPaint::FontMetrics& SkGlyphCache::getFontMetricsYByFontID(uint32_t fontID) const {
    SkScalerContext* ctx = fScalerContext;

    SkPaint::FontMetrics metricsY;
    fScalerContext->getFontMetricsByFontID(NULL, &metricsY, fontID);
    return metricsY;
}
