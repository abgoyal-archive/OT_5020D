...
//    fb_addr      = (void *)((UINT32)lcdbase + fb_size);
    ...
    /*
       If HAVE_CACHE_PL310 is enalbe, *must* flush <intermediat buffer> from
       L2 cache back to DDR after memset(clear all FB to black) with
       video mode LCD.Otherwise boot logo maybe displayed mess.
	 */
    arch_clean_cache_range((void *)((UINT32)lcdbase + DISP_GetFBRamSize()),
        DISP_GetVRamSize() - DISP_GetFBRamSize());
#endif
