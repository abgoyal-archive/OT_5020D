...
protected:
    ...
    /** M: add to support complex language @{ */
public:
    virtual uint32_t getFontID(){
        return 1;
    }
    virtual void getFontMetrics(uint16_t* x_ppem, uint16_t* y_ppem, SkFixed* x_scale, SkFixed* y_scale) {
        *x_ppem = 1;
        *y_ppem = 1;
        *x_scale = SK_Fixed1;
        *y_scale = SK_Fixed1;
...
/** M: add to support complex language @{ */
uint32_t SkScalerContext::getCharFontID(SkUnichar uniChar, uint32_t* glyphBase) {
    SkScalerContext* ctx = this;
    unsigned glyphID;
    ...
    for (;;) {
        glyphID = ctx->generateCharToGlyph(uniChar);
        if (glyphID) {
            break;  // found it
        }
        ctx = ctx->getNextContext();
        if (NULL == ctx) {
            ctx = this;
            break;   // no more contexts, return missing glyph
        }
    }
    if (glyphBase != NULL) {
        (*glyphBase) = ctx->getBaseGlyphCount();
    }
    uint32_t id = ctx->getFontID();
    return id;

}

uint16_t SkScalerContext::getGlyphIDbyIndexAndFont(uint16_t glyphIndex, uint32_t fontID) {
    SkScalerContext* ctx = this;
    unsigned glyphID;
    for (;;) {
        if (NULL == ctx) {
            SkDebugf("!!!!!!!!!!!!!!!!!!!!!");
            SkDebugf("SkScalerContext::getGlyphIDbyIndexAndFont fontID=%d glyphIndex=%d", fontID, glyphIndex);
            ctx = this;
            break;   // no more contexts, return missing glyph
        }
        if (ctx->getFontID() == fontID) {
            break;  // found it
        }
        ctx = ctx->getNextContext();
    }
    uint16_t id = ctx->getBaseGlyphCount()+glyphIndex;
    return id;

}


uint16_t SkScalerContext::getCharGlyphIndex(uint16_t uniChar) {
    SkScalerContext* ctx = this;
    uint16_t glyphID;
    for (;;) {
        glyphID = ctx->generateCharToGlyph(uniChar);
...
uint32_t SkScalerContext::getFontID() {
    uint32_t id = 1;
    return id;

}

void SkScalerContext::getFontMetrics(uint16_t* x_ppem, uint16_t* y_ppem, SkFixed* x_scale, SkFixed* y_scale) {
    *x_ppem = 1;
    *y_ppem = 1;
    *x_scale = SK_Fixed1;
    *y_scale = SK_Fixed1;
...
void SkScalerContext::getFontMetricsByFontID(uint16_t* x_ppem, uint16_t* y_ppem, SkFixed* x_scale, SkFixed* y_scale, uint32_t fontID) {
    SkScalerContext* ctx = this;

    for (;;) {
        if (fontID == ctx->getFontID()) {
            break;
        }
        ctx = ctx->getNextContext();
        if (NULL == ctx) {
    ...
    ctx->getFontMetrics(x_ppem, y_ppem, x_scale, y_scale);
}

void SkScalerContext::getFontMetricsByFontID(SkPaint::FontMetrics* mx,
                                     SkPaint::FontMetrics* my,
                                     uint32_t fontID) {
    SkScalerContext* ctx = this;
