...
//for HAC Setting
	  case SET_HAC_ON:
	  {
         PRINTK_AUDDRV("AudDrv SET_HAC_ON arg:%u \n",arg);
		 mutex_lock(&gamp_mutex);
         Sound_Hac_Turnon();
		 mutex_unlock(&gamp_mutex);
         break;
...
//===========================================================================================
static ssize_t set_attr_hacctrl(struct device *dev,
...
static DEVICE_ATTR(hac_en, S_IWUSR, NULL, set_attr_hacctrl);
...
static int AudDrv_mod_init(void)
{
   ...
   // cat /proc/audio
	create_proc_read_entry("audio",
         	  ...
         	  AudDrv_Read_Procmem,
  		                       NULL);
   sysfs_create_file(&AudDrv_audio_device.this_device->kobj, &dev_attr_hac_en);
...
static void  AudDrv_mod_exit(void)
{
   ...
   sysfs_remove_file(&AudDrv_audio_device.this_device->kobj, &dev_attr_hac_en);
