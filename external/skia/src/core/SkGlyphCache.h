...
public:
    size_t getFontID(SkUnichar uniChar, size_t* glyphBase);
    uint16_t unicharToGlyphIndex(uint16_t uniChar);
    const SkGlyph& getGlyphIDAdvanceByFontID(uint16_t glyphIndex, uint32_t fontID);
    const SkGlyph& getGlyphIDMetricsByFontID(uint16_t glyphIndex, uint32_t fontID);
    const SkPaint::FontMetrics& getFontMetricsYByFontID(uint32_t fontID) const;
