...
void SkCanvas::drawText(const void* text, size_t byteLength, const SkScalar* xOffset, const SkScalar* yOffset,
                        SkScalar x, SkScalar y, const SkPaint& paint) {
    LOOPER_BEGIN(paint, SkDrawFilter::kText_Type)

    while (iter.next()) {
        SkDeviceFilteredPaint dfp(iter.fDevice, looper.paint());
        iter.fDevice->drawText(iter, text, byteLength, xOffset, yOffset, x, y, dfp.paint());
    }
