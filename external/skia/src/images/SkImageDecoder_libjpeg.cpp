...
private:
    ...
    bool onDecodeHW(SkBitmap* bm, uint8_t* srcBuffer, uint32_t srcSize, int srcFD);
...
bool SkJPEGImageDecoder::onDecodeParser(SkBitmap* bm, Mode mode, uint8_t* srcBuffer, uint32_t srcSize, int srcFD)
{
                                   ...
                                   (void *)&outInfo, sizeof(outInfo), NULL))
    {
        if (SkImageDecoder::kDecodeBounds_Mode != mode) {
            XLOGW("ERROR : mHalJpeg() - JPEG Decoder get information\n");
        }
        return false;
    }
    ...
    if(reuseBitmap)
    {
        ...
        {
            if (MHAL_NO_ERROR != mHalJpeg(MHAL_IOCTL_JPEG_DEC_START, NULL, 0, NULL, 0, NULL))
            {
...
bool SkJPEGImageDecoder::onDecodeHW(SkBitmap* bm, uint8_t* srcBuffer, uint32_t srcSize, int srcFD)
{
...
bool SkJPEGImageDecoder::onDecode(SkStream* stream, SkBitmap* bm, Mode mode) {
    ...
    else 
    {
        ...
        if(true == onDecodeParser(bm, mode, tSrc, rSize, tAllocator.getFD()))
        {
            ...
            else
            {
                ...
                {
                    XLOGD("SkiaJpeg:TRY_SW_PATH mSize %x, rSize %x, align rSize %x, skSize %x!!\n", sLength, rSize, (rSize + 31) & (~31), stream->getLength());
                    if(rSize > MAX_HEADER_SIZE)
                    {
#ifdef USE_SKJPGSTREAM						
...
#if MTK_JPEG_HW_DECODER
bool MDPResizer(void* src, int width, int height, SkScaledBitmapSampler::SrcConfig sc, SkBitmap* bm)
{
    if((NULL == bm))
    {
...
if(!fFirstTileDone || fUseHWResizer)
{
    ...
    do
    {
        result = MDPResizer(hwBuffer, width, height, sc, bitmap);
