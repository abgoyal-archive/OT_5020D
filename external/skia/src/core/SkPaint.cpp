...
/** M: Add API to process glyph or character by font id @{ */
static const SkGlyph& sk_getMetrics_glyph_next_by_fontID(SkGlyphCache* cache,
                                               const char** text, uint32_t fontID) {
    SkASSERT(cache != NULL);
    SkASSERT(text != NULL);
    ...
    ptr += 1;
    *text = (const char*)ptr;
    return cache->getGlyphIDMetricsByFontID(glyphID, fontID);
}

static const SkGlyph& sk_getMetrics_glyph_prev_by_fontID(SkGlyphCache* cache,
                                               const char** text, uint32_t fontID) {
    SkASSERT(cache != NULL);
    SkASSERT(text != NULL);
...
static const SkGlyph& sk_getAdvance_glyph_next_by_fontID(SkGlyphCache* cache,
                                               const char** text, uint32_t fontID) {
    SkASSERT(cache != NULL);
    SkASSERT(text != NULL);
    ...
    ptr += 1;
    *text = (const char*)ptr;
    return cache->getGlyphIDAdvanceByFontID(glyphID, fontID);
}

static const SkGlyph& sk_getAdvance_glyph_prev_by_fontID(SkGlyphCache* cache,
                                               const char** text, uint32_t fontID) {
    SkASSERT(cache != NULL);
    SkASSERT(text != NULL);
...
SkMeasureCacheByFontProc SkPaint::getMeasureCacheProcByFontID(TextBufferDirection tbd,
                                                bool needFullMetrics) const {
    static const SkMeasureCacheByFontProc gMeasureCacheProcs[] = {
        NULL,
        NULL,
        sk_getMetrics_glyph_next_by_fontID,

        NULL,
        NULL,
        sk_getMetrics_glyph_prev_by_fontID,

        NULL,
        NULL,
        sk_getAdvance_glyph_next_by_fontID,

        NULL,
        NULL,
        sk_getAdvance_glyph_prev_by_fontID
    };
    ...
    SkASSERT(index < SK_ARRAY_COUNT(gMeasureCacheProcs));
    SkASSERT(gMeasureCacheProcs[index] != NULL);
    return gMeasureCacheProcs[index];
}


uint32_t SkPaint::getFontID(SkUnichar uniChar, uint32_t* glyphBase) {
    SkAutoGlyphCache    autoCache(*this, NULL);
    SkGlyphCache*       cache = autoCache.getCache();
    uint32_t id = (uint32_t)cache->getFontID(uniChar, glyphBase);
    return id;
}


int SkPaint::textToGlyphIndex(const void* textData, size_t length, uint16_t glyphs[]) const {
    if (length == 0) {
        return 0;
    }
    ...
    if (NULL == glyphs) {
        switch (this->getTextEncoding()) {
        case kUTF8_TextEncoding:
            return SkUTF8_CountUnichars((const char*)textData, length<<2);
        case kUTF16_TextEncoding:
            return SkUTF16_CountUnichars((const uint16_t*)textData,
                                         length);
        case kGlyphID_TextEncoding:
    ...
    uint16_t* text = (uint16_t*)textData;
    uint16_t*   gptr = glyphs;

    for (size_t i = 0; i < length; i ++) {
        *gptr++ = cache->unicharToGlyphIndex(text[i]);
    }

    return length;
}

void SkPaint::getFontMetricsByFontID(uint16_t* x_ppem, uint16_t* y_ppem, SkFixed* x_scale, SkFixed* y_scale, uint32_t fontID) const {
    SkScalar                            scale = 0;
    SkAutoRestorePaintTextSizeAndFrame  restore(this);
    ...
    cache->getFontMetricsByFontID(x_ppem, y_ppem, x_scale, y_scale, fontID);
}


SkScalar SkPaint::getFontMetricsByFontID(FontMetrics* metrics, uint32_t fontID, SkScalar zoom) const {
    SkScalar                            scale = 0;
    SkAutoRestorePaintTextSizeAndFrame  restore(this);
    ...
    SkAutoGlyphCache    autoCache(*this, zoomPtr);
    SkGlyphCache*       cache = autoCache.getCache();
    (*metrics) = cache->getFontMetricsYByFontID(fontID);
