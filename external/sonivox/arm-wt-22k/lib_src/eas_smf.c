...
static EAS_RESULT SMF_HandleSeekTableEvent (S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream, EAS_INT parserMode, EAS_U32 position);
		...
		case PARSER_DATA_LOCATE:
			{
				...
				if((result = SMF_HandleSeekTableEvent(pEASData, pSMFData, pSMFData->nextStream, eParserModeLocate, pSMFData->seekTime)) != EAS_SUCCESS)
			  {
			        LOGV("SMF_SetData:  PARSER_DATA_LOCATE, call SMF_HandleSeekTableEvent return %d", result);
			    	return result;						
				}	
				/* are there any more events to parse? */
			    if (pSMFData->nextStream)
...
static EAS_RESULT SMF_ParseSysEx (S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream, EAS_U8 f0, EAS_INT parserMode)
{
    ...
    if( parserMode == eParserModeMetaData){
		...
		pSMFData->pMST->MST_UpdateSpecialMsgTable(pSMFData->pMST, (void*)&sysEvent, sysEvent.dataNum);
	}
	if(data) free(data);
...
static EAS_RESULT SMF_ParseEvent (S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream, EAS_INT parserMode)
{
    ...
    else
    {
		...
		if( parserMode == eParserModeMetaData){
            ...
            pSMFData->pMST->MST_UpdateSpecialMsgTable(pSMFData->pMST, (void*)&channelEvent, channelEvent.dataNum);
                }
...
static EAS_RESULT SMF_HandleSeekTableEvent (S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream, EAS_INT parserMode, EAS_U32 position)
{
    ...
    eventList  = pSMFData->pMST->mstdata.pSpecailMSGTableBegin;
    eventCount = pSMFData->pMST->MST_GetSpecialMsgCount(pSMFData->pMST, position);
