...
#if defined(JRD_BATTERY_ID_SUPPORT)
...
#define CHG_MIN_VALID_BATT_ID  650		//24/34*1024 = 723; 723*0.9=650;723*1.1=795
...
#define BAT_NTC_TSM_1  2		// for byd battery.
...
extern void display_black_picture(void);		//tao.zhang@tcl.com  2013-01-28. reduce shink time.

#if BATT_ID_CHECK_SUPPORT
void auxadc_cal_prepare(void);
...
{    
    ...
    #if BATT_ID_CHECK_SUPPORT
     printf("BATT_ID_CHECK_SUPPORT = yes\n");
    check_battery_id();
    if(BMT_status.bat_id_valid == 0)
     ...
     #else
	printf("BATT_ID_CHECK_SUPPORT = no\n");
    #endif
    ...
    if ((upmu_is_chr_det() == KAL_TRUE))    
    {
        ...
        display_black_picture();		//tao.zhang@tcl.com  2013-01-28. reduce shink time.
        while (1) 
        {
                ...
                || mtk_wdt_boot_check()==WDT_BY_PASS_PWK_REBOOT || g_boot_arg->boot_reason==BR_TOOL_BY_PASS_PWK || g_boot_menu==true )
            {
                ...
                if(back_to_charging_animation_flag == false)
                {
                    ...
                    sc_mod_exit();
		    mdelay(50);   //tao.zhang@tcl.com 2013-02-06 modify. wait the pll to stable.
                    return;
                }
            ...
            if(print_msg==FALSE)
            {    
            //2013-01-02 huanhuan.guo "Change charging screen display" begin
                //#ifdef ANIMATION_NEW 	
                BAT_CheckBatteryStatus();  
                //#endif
			//2013-01-02 huanhuan.guo "Change charging screen display" end
                lk_charging_display();
