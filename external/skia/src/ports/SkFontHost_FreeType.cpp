...
// merge MTK patch JRDSH77_CU_JB_ALPS.JB.MP.V1_P235 by yijun.liu@tcl.com at 2013-4-8 begin.
// hand-tuned value to reduce outline embolden strength
    ...
    #endif
#endif
// merge MTK patch JRDSH77_CU_JB_ALPS.JB.MP.V1_P235 by yijun.liu@tcl.com at 2013-4-8 end.
...
public:
    ...
    void getFontMetrics(uint16_t* x_ppem, uint16_t* y_ppem, SkFixed* x_scale, SkFixed* y_scale);
};
...
/** M: add for complex language support @{ */

void SkScalerContext_FreeType::getFontMetrics(uint16_t* x_ppem, uint16_t* y_ppem, SkFixed* x_scale, SkFixed* y_scale) {
    ...
    if (x_ppem) {
        *x_ppem = fFace->size->metrics.x_ppem;
    }
    if (y_ppem) {
        *y_ppem = fFace->size->metrics.y_ppem;
    }
    if (x_scale) {
        *x_scale = fFace->size->metrics.x_scale;
    }
    if (y_scale) {
        *y_scale = fFace->size->metrics.y_scale;
    }
}

uint32_t SkScalerContext_FreeType::getFontID() {
    uint32_t id = this->fFaceRec->fFontID;
    return id;
